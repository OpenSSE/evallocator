...
def plot_file(filename, label, add_overflow_overhead):
    ...
    with open(filename) as souce:
        ...
        for experiment in data:  # iterate over the experiments
            ...
            base_overhead = (experiment["parameters"]["m"])/float(n)
            ...
            ax2.plot(overhead, stat_mean,
                     label=l, color=color, marker="x")
            ax1.plot(overhead, stat_max, dashes=[
                6, 2], color=color, marker="x")
            ax4.plot(overhead, stat_mean,
                     label=l, color=color, marker="x")
            ax3.plot(overhead, stat_max, dashes=[
                     6, 2], color=color, marker="x")
