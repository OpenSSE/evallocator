    ...
    F: FnMut(usize, usize),
{
    assert!(
        m % block_size == 0,
        "Invalid block size: the number of buckets (m) must be divisible by the block size"
    );
...
pub fn alloc(
    ...
    let buckets = alloc_progress(n, m, max_len, block_size, progress_callback);
...
pub fn experiment(
    ...
    let rand_alloc = alloc(n, m, max_len, block_size, show_progress);
    let size = rand_alloc.iter().sum();
    let max_load = rand_alloc.iter().fold(0, |m, x| m.max(*x));
    let load_modes = compute_modes(rand_alloc.into_iter(), max_load);
    let overflows = (0..=overflow_max)
    ...
    F: Fn(usize) + Send + Sync,
{
    ...
    let results: Vec<ExperimentResult> = (0..iterations)
        .into_par_iter()
        .map(|_| {
            let r = experiment_progress(n, m, max_len, block_size, overflow_max, progress_callback);
