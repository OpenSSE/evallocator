...
where
    T: Copy
        ...
        + AddAssign
        + Shr<u8>
        + BitOrAssign<<T as std::ops::Shr<u8>>::Output>,
{
...
fn alloc(n: usize, m: usize, max_len: usize) -> Vec<usize> {
    assert!(m > max_len);
...
fn main() {
    ...
    let allocation = alloc(n, m, max_len);

    let total:usize = allocation.iter().sum();
    let max_load:usize = allocation.iter().fold(0, |m,x| m.max(*x));

    println!("Total {}", total);
    println!("Overhead {}", (total as f64)/(n as f64)-1.);
    println!("Max {}", max_load);
    // println!("{:?}", allocation);
}
